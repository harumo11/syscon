...
public:
     ...
     * lowpass1::set_filter_params() to pass the filter parameter before you use this filter.
     */
    lowpass1() {};

    /**
     * @brief The constructor of First-order system.
     * This constructor takes all the parameters that needed to start using the filter.
     *
     ...
     * This value is observed by a sensor, etc., and includes high frequency components.
     *
     * @return observed_x without high frequency components.
     */
    double step(const double observed_x)
    {
        Require(this->sampling_period != 0, "Coefficients of filter (sampling period) is not valid. Did you set filter parameters?");
        ...
        double y = this->filter_coefficients.at(0) * this->x.at(0)
            + this->filter_coefficients.at(1) * this->x.at(1)
            + this->filter_coefficients.at(2) * this->y_previous;
    ...
    {
        return { this->filter_coefficients, { this->sampling_period, this->time_constant } };
    };
...
private:
    ...
    /**
     * @brief Calculates three coefficients of filter from cut-off frequency and sampling frequency.
     *
     * @param cutoff_freq cut-off frequency [Hz].
     * @param sample_freq samplilng frequency to observe data [Hz].
     *
     * @return Three coefficients of filter. Values for c1 to c3 in the below equation.
     *   y[t] = c1 * x[t] + c2 * x[t-1] + c3 * y[t-1]
     */
    std::vector<double> calc_filter_coefficients(const double cutoff_freq, const double sample_freq)
    {
        ...
        this->sampling_period = 1.0 / sample_freq;
        ...
        calculated_filter_coefficients.at(0) = this->sampling_period / (this->sampling_period + 2 * this->time_constant);
        calculated_filter_coefficients.at(1) = this->sampling_period / (this->sampling_period + 2 * this->time_constant);
        calculated_filter_coefficients.at(2) = -1 * (this->sampling_period - 2 * this->time_constant) / (this->sampling_period + 2 * this->time_constant);
